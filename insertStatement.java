    ...
    public void execute() throws DatabaseException, DeadlockException {
        try {
            ...
            if (table.primaryKeyColumn() == null){
                table.getDB().append(DBMS.getTxn(), row.getKey(), row.getData());
                
            }
            else {
                 if (table.getDB().putNoOverwrite(DBMS.getTxn(), row.getKey(), row.getData()) == OperationStatus.KEYEXIST){
                    Column col = table.primaryKeyColumn();
                    throw new DatabaseException("There is an existing row with the specified primary key. Could not insert row.");
                    }
