...
public class TableIterator extends RelationIterator {
    ...
    public boolean next() throws DeadlockException, DatabaseException {
        try {
            ...
            System.err.println("error occurred while using cursor");
        }
        return false;
    }
    ...
    public Object getColumnVal(int colIndex) {
        ...
        if (column.isPrimaryKey()) {  
            dataIn = new TupleInput(key.getData());
            dataIn.mark(0);
            columnSize = dataIn.readInt();
        }
        //not a primary key
        else {
            dataIn = new TupleInput(data.getData());
            dataIn.mark(0);
            int startOffset = colIndex;
            int endOfOffset = table.numColumns();

            if (table.primaryKeyColumn() != null) {
                endOfOffset = endOfOffset - 1;
                if (table.primaryKeyColumn().getIndex() < colIndex) {
                    startOffset = startOffset - 1;
                }
            }
            if (startOffset > 0) {
                dataIn.skip((startOffset - 1) * 4);
                startBytePosition = dataIn.readInt();
                endBytePosition = dataIn.readInt();
            } else {
                startBytePosition = 0;
                endBytePosition = dataIn.readInt();
            }
            dataIn.reset();
            dataIn.skip(startBytePosition);
            dataIn.skip(4 * endOfOffset);
            columnSize = (endBytePosition - startBytePosition);
        }
       
        if (column.getType() == 0) {
            columnValue = dataIn.readInt();
        } else if (column.getType() == 1) {
            columnValue = dataIn.readDouble();
        } else {
            columnValue = dataIn.readBytes(columnSize);
        }
        try {
            dataIn.close();
        } catch (IOException e) {
